...
import { QuickStatCard, AlertBanner } from '../shared/Cards';
...
export function OverviewTab({ session, stats }: OverviewTabProps) {
  const allInsights = session.perspectives?.flatMap(p => p.key_insights || []) || [];
  const allWarnings = session.perspectives?.flatMap(p => p.warnings || []) || [];
  const allRecommendations = session.perspectives?.flatMap(p => p.recommendations || []) || [];
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <QuickStatCard
          ...
          color="blue"
        >
          <ConfidenceDistribution high={stats.highConfidence} medium={stats.medConfidence} low={stats.lowConfidence} />
        </QuickStatCard>
        <QuickStatCard
        ...
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-4">
          ...
          <ul className="space-y-1">
            {allWarnings.slice(0, 4).map((w, i) => (
              <li key={i} className="text-sm text-amber-800 pl-6 relative before:content-['•'] before:absolute before:left-2 before:text-amber-400">
                {w}
              </li>
            ))}
          </ul>
        ...
        <ThemedSection title="Key Insights" count={allInsights.length}>
          <ul className="space-y-2">
            {allInsights.slice(0, 6).map((insight, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-slate-700 pl-3 border-l-2 border-emerald-400 py-1">
                {insight}
              </li>
            ))}
          </ul>
      ...
      {/* Gaps */}
      {stats.gaps > 0 && (
        <ThemedSection title="Research Gaps" count={stats.gaps}>
          <div className="space-y-2">
            {session.gaps?.slice(0, 3).map((gap, i) => (
