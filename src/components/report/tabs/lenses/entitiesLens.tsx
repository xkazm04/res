...
import { PersonIcon, GridIcon, TargetIcon, ClockIcon, UsersIcon } from '../../shared/Icons';
...
export const entitiesLens = defineLens<EntitiesData>({
  id: 'entities',
  label: 'Entities',
  icon: UsersIcon,
  description: 'People, organizations, locations, and events mentioned in research',
  order: 3,
  filters: [],

  getBadgeCount: (session, stats) => stats.entities,
  ...
  extract: (session): EntitiesData => {
    const entities = session.entities || [];

    // Group entities by type
    const grouped: EntityGroups = {
      person: [],
      organization: [],
      location: [],
      event: [],
...
function EntitiesLensView({ data }: LensRenderProps<EntitiesData>) {
  const { entities, grouped } = data;
  ...
  return (
    <div className="space-y-4">
      ...
      {grouped.organization.length > 0 && (
        <ThemedSection
          title="Organizations"
          count={grouped.organization.length}
          collapsible
          defaultExpanded
        >
...
function EntityList({ entities }: { entities: KnowledgeEntity[] }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
      {entities.map((entity) => (
        <EntityCardLens key={entity.id} entity={entity} />
      ))}
    </div>
  );
}

function EntityCardLens({ entity }: { entity: KnowledgeEntity }) {
      ...
      actions={
        <span
          className={`text-[10px] px-1.5 py-0.5 rounded-full ${
            isRadar ? 'bg-slate-700 text-slate-400' : 'bg-slate-100 text-slate-600'
          }`}
        >
