 ...
 */
export function SummaryScene({ frame, fps, isRadar, format, confidence, findings, sources }: SummarySceneProps) {
  const isMobile = format === 'mobile';

  // Animation timings
  const bgProgress = spring({ frame, fps, delay: 0, durationFrames: 28, easing: easeOutCubic });
  const mainProgress = spring({ frame, fps, delay: 3, durationFrames: 26, easing: easeOutExpo });
  const ringProgress = spring({ frame, fps, delay: 6, durationFrames: 32, easing: easeOutCubic });
  const statsProgress = spring({ frame, fps, delay: 16, durationFrames: 22, easing: easeOutCubic });
  const badgeProgress = spring({ frame, fps, delay: 28, durationFrames: 20, easing: easeOutQuart });
  const breathe = pulse(frame, fps, 0.5);
  ...
  return (
    <div className={`absolute inset-0 flex flex-col items-center overflow-hidden ${isMobile ? 'justify-start pt-12' : 'justify-center'}`}>
      ...
      <div
        className={`absolute ${isMobile ? 'w-64 h-64' : 'w-[400px] h-[400px]'} rounded-full ${isRadar ? 'bg-gradient-radial from-cyan-500/15 via-transparent to-transparent' : 'bg-gradient-radial from-stone-400/10 via-transparent to-transparent'}`}
        style={{ opacity: bgProgress }}
      />

      {/* Animated rings */}
      <div
        className={`absolute ${isMobile ? 'w-32 h-32' : 'w-40 h-40'} rounded-full border-2 ${isRadar ? 'border-cyan-500/20' : 'border-stone-400/20'}`}
        style={{ opacity: ringProgress * 0.5, transform: `scale(${0.6 + ringProgress * 0.5 * breathe})` }}
      />
      <div
        className={`absolute ${isMobile ? 'w-48 h-48' : 'w-56 h-56'} rounded-full border ${isRadar ? 'border-cyan-500/10' : 'border-stone-400/10'}`}
      ...
      <div className="relative" style={{ opacity: mainProgress, transform: `scale(${0.5 + mainProgress * 0.5})` }}>
        ...
        <div className="relative text-center">
          <div className={`${isMobile ? 'text-5xl' : 'text-6xl'} font-bold tabular-nums ${confidenceColor}`}>
            <AnimatedCounter value={confidence} frame={frame} fps={fps} delay={6} suffix="%" />
          </div>
          <div className={`${isMobile ? 'text-sm' : 'text-base'} mt-1 font-medium ${isRadar ? 'text-white/90' : 'text-stone-700'}`}>
            Overall Confidence
          </div>
        </div>
      </div>
      ...
      {/* Stats row */}
      <div
        className={`flex ${isMobile ? 'gap-4 mt-6' : 'gap-6 mt-8'}`}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`font-semibold ${isMobile ? 'text-xs' : 'text-sm'}`}>Analysis Complete</span>
        </div>
      </div>
...
function StatCard({ value, label, frame, fps, delay, isRadar, isMobile, color }: StatCardProps) {
  return (
    <div className={`text-center ${isMobile ? 'px-4 py-2' : 'px-5 py-2.5'} rounded-xl ${isRadar ? 'bg-slate-800/60 border border-slate-700' : 'bg-white/80 border border-stone-200'}`}>
      <div className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold tabular-nums ${color}`}>
        <AnimatedCounter value={value} frame={frame} fps={fps} delay={delay} />
      </div>
      <div className={`text-[10px] mt-0.5 ${isRadar ? 'text-slate-400' : 'text-stone-500'}`}>{label}</div>
    </div>
  );
}
