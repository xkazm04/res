...
export interface VizColorsReturn {
  /** Current theme name */
  theme: ReportTheme;
...
/**
 * Get visualization colors and utilities for a given theme.
 *
 * This is a pure function with no React dependencies - use it directly
 * instead of the useVisualizationTheme hook when you don't need semantic intent.
 *
 * @example
 * ```tsx
 * function MyVisualization({ theme }: { theme: 'radar' | 'swiss' }) {
 *   const { colors, isRadar, getConfidenceColor, cardClasses } = getVizColors(theme);
 *
 *   return (
 ...
 * ```
 */
export function getVizColors(theme: ReportTheme): VizColorsReturn {
